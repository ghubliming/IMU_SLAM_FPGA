    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    //在构造函数中初始化文件流
    // 打开日志文件
    mTrackLogFile.open("MapPointTracks.csv");
    if (!mTrackLogFile.is_open()) {
        std::cerr << "Failed to open MapPointTracks.csv!" << std::endl;
        exit(-1);
    }
    // 写入CSV头
    mTrackLogFile << "timestamp,frame_id,mp_id_persistent,px_x,px_x,world_x,world_y,world_z\n";
}
...
void Tracking::PreintegrateIMU()
{
    ...
    while(true)
    {
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                cout.precision(17);
                //检查IMU时间，弹出不在两帧之间的imu数据
                if(m->t<mCurrentFrame.mpPrevFrame->mTimeStamp-mImuPer)//imu早了
                {
                    mlQueueImuData.pop_front();
                }
                else if(m->t<mCurrentFrame.mTimeStamp-mImuPer)//imu合适
                {
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if((i==0) && (i<(n-1)))//第一个imu帧且至少有两个imu帧，求第一帧到上一图像帧的平均加速度，用下一imu帧减去当前imu帧的得到加速度的“加速度”
        {
            //相邻imu时间间隔
        ...
        else if(i<(n-1))//第一帧和倒数第二帧之间的帧，求平均值就行
        {
            acc = (mvImuFromLastFrame[i].a+mvImuFromLastFrame[i+1].a)*0.5f;
            angVel = (mvImuFromLastFrame[i].w+mvImuFromLastFrame[i+1].w)*0.5f;
            tstep = mvImuFromLastFrame[i+1].t-mvImuFromLastFrame[i].t;
        }
        else if((i>0) && (i==(n-1)))//倒数第二个imu帧，同样需要补偿
        {
            float tab = mvImuFromLastFrame[i+1].t-mvImuFromLastFrame[i].t;
        ...
        //mpImuPreintegratedFromLastKF 当前帧和上一关键帧
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(acc,angVel,tstep);
        // pImuPreintegratedFromLastFrame当前帧和上一帧
        pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(acc,angVel,tstep);
    }

    //查看图像帧时间
    std::cout << "图像时间" << mCurrentFrame.mpPrevFrame->mTimeStamp  <<  " and " <<  mCurrentFrame.mTimeStamp << std::endl;
    std::cout << "dP " << pImuPreintegratedFromLastFrame->dP << std::endl;
    //std::cout << "GetDeltaPosition  " << pImuPreintegratedFromLastFrame->GetDeltaPosition(pImuPreintegratedFromLastFrame->b) << std::endl;
    //std::cout << "dV   " << pImuPreintegratedFromLastFrame->dV << std::endl;
    std::cout << "dR  " << std::endl << pImuPreintegratedFromLastFrame->dR << std::endl;
    //std::cout << "dax " << pImuPreintegratedFromLastFrame->b.bax <<  " day " << pImuPreintegratedFromLastFrame->b.bay << " daz " << pImuPreintegratedFromLastFrame->b.baz  << std::endl; 
    //std::cout << "dwx " << pImuPreintegratedFromLastFrame->b.bwx <<  " dwy " << pImuPreintegratedFromLastFrame->b.bwy << " dwz " << pImuPreintegratedFromLastFrame->b.bwz << std::endl << std::endl; 
    //std::cout  << pImuPreintegratedFromLastFrame->b.bax <<  " " << pImuPreintegratedFromLastFrame->b.bay << " " << pImuPreintegratedFromLastFrame->b.baz  << " "; 
    //std::cout  << pImuPreintegratedFromLastFrame->b.bwx <<  " " << pImuPreintegratedFromLastFrame->b.bwy << " " << pImuPreintegratedFromLastFrame->b.bwz << std::endl; 
...
void Tracking::Track()
{
    ...
    //std::cout << "mCurrFrame.mImuBias.bax  mpLastKeyFrame->GetImuBias() " << mCurrentFrame.mImuBias.bax << std::endl;
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        {
            //============new code====================
            double time = mCurrentFrame.mTimeStamp;//时间
    ...
    else
    {
        ...
        if(!mCurrentFrame.mpReferenceKF)
            mCurrentFrame.mpReferenceKF = mpReferenceKF;
             ...
             //============new code====================
        double time = mCurrentFrame.mTimeStamp;//时间
        // 用户代码插入点：在更新mpLastFrame之前获取当前帧和上一帧的位姿
        Frame prev_frame = mLastFrame;    // 上一帧（未更新）
        Frame curr_frame = mCurrentFrame;  // 当前帧（已计算位姿）

        // 提取位姿（Sophus::SE3f格式）
        Sophus::SE3f T_prev = prev_frame.GetPose();
        Sophus::SE3f T_curr = curr_frame.GetPose();

        // 计算相对变换
        Sophus::SE3f T_curr_prev = T_curr * T_prev.inverse();
        Eigen::Vector3f dP = T_curr_prev.translation();
        Eigen::Matrix3f dR = T_curr_prev.rotationMatrix();
    ...
    // 当跟踪成功时记录数据（约在 Track() 函数末尾）
    if (mState == OK) {
        const int frame_id = mCurrentFrame.mnId;
        const double timestamp = mCurrentFrame.mTimeStamp;
    
        for (int i = 0; i < mCurrentFrame.N; ++i) {
            // 从当前帧的地图点列表中获取 pMP
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];  
    
            if (pMP && !pMP->isBad()) {  // 检查有效性
                Eigen::Vector3f pos = pMP->GetWorldPos();
                const cv::KeyPoint& kp = mCurrentFrame.mvKeysUn[i];

                const float depth = mCurrentFrame.GetDepths()[i];  // 直接获取双目深度
                if (depth <= 0) continue;  // 跳过无效深度
    
                // 写入文件
                std::unique_lock<std::mutex> lock(mMutexLog);//原子操作
                mTrackLogFile   << timestamp << "," // 时间戳
                                << frame_id  << ","   // 图像帧唯一ID
                                << pMP->mnId << ","  //地图点的唯一ID
                                << kp.pt.x << "," << kp.pt.y << "," //该地图点在图像帧的像素坐标
                                << pos.x() << ","  //地图点空间坐标
                                << pos.y() << ","
                                << pos.z() << ","
                                << depth   << "\n";  
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
            ...
            {
                cout << "not enough acceleration   （当前帧的平均加速度和上一帧的平均加速度之差小于0.5）" << endl;
                return;
            }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //std::cout << "mCurrentFrame.mImuBias.bax" << mCurrentFrame.mImuBias.bax << std::endl;
            }
        }
    }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mnLastKeyFrameId = mCurrentFrame.mnId;
    //IMU::Bias aaa = mpLastKeyFrame->GetImuBias();
    //cout << "aaa " << aaa.bax << std::endl;
    mpLastKeyFrame = pKF;
}
